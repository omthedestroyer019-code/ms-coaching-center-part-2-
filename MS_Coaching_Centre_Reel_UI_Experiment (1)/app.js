...
function searchBox(label='Search this section'){return `<div class="search-card premium-search"><span class="search-glyph" aria-hidden="true">⌕</span><input id="sectionSearch" class="search-input" placeholder="${label}" autocomplete="off"></div>`}
...
function renderDashboard(){
  ...
  const batches=roleBatches();
  const today=new Date().toISOString().slice(0,10);
  const todaysAttendance=state.attendance.filter(a=>students.some(s=>s.id===a.student_id)&&a.date===today).length;
  const pending=state.fees.filter(f=>students.some(s=>s.id===f.student_id)&&f.status!=='paid').length;
  const activeStudents=students.filter(s=>(s.status||'active')==='active').length;
  const freshNotices=visibleNotices().slice(0,3);
  const roleMessage=state.currentUser.role==='teacher'
    ?'Run attendance, publish notes and keep every assigned batch on track from one command centre.'
    :state.currentUser.role==='student'
      ?'Track your attendance, fees, materials and results from one focused control panel.'
      :'Manage students, teachers, fees and notices with a cinematic Max Pro dashboard.';
  const fourth=state.currentUser.role==='teacher'
    ?`<div class="stat-card metric-card"><span class="metric-icon">✓</span><strong>${todaysAttendance}</strong><span>Today Attendance</span><small>Marked today</small></div>`
    :`<div class="stat-card metric-card"><span class="metric-icon">₹</span><strong>${pending}</strong><span>Pending Fees</span><small>Needs follow-up</small></div>`;
  const focusCards=[
    ['Attendance', todaysAttendance, 'Today marked', 'attendance', '✓'],
    ['Materials', roleMaterials().length, 'Visible notes', 'materials', '▤'],
    ['Notices', visibleNotices().length, 'Active updates', 'notices', '!']
  ].filter(([,,,section])=>isAllowedSection(section));
  return `<div class="hero-card reel-hero maxpro-hero"><div class="reel-hero-copy"><span class="reel-kicker">M.S PREMIUM COMMAND CENTER</span><h3>Welcome back, ${state.currentUser.full_name}</h3><p>${roleMessage}</p><div class="reel-mini-row"><span>${activeStudents} active students</span><span>${batches.length} live batches</span><span>${state.currentUser.role.toUpperCase()} MODE</span></div></div><div class="reel-hero-visual" aria-hidden="true"><span class="reel-orb-core"></span><span class="reel-orb-ring reel-ring-one"></span><span class="reel-orb-ring reel-ring-two"></span><span class="reel-orb-chip chip-a">LIVE</span><span class="reel-orb-chip chip-b">MS</span></div></div><div class="grid pro-metric-grid"><div class="stat-card metric-card"><span class="metric-icon">◉</span><strong>${students.length}</strong><span>Students</span><small>${activeStudents} active</small></div><div class="stat-card metric-card"><span class="metric-icon">◇</span><strong>${state.currentUser.role==='student'?1:state.teachers.length}</strong><span>Teachers</span><small>Faculty view</small></div><div class="stat-card metric-card"><span class="metric-icon">▦</span><strong>${batches.length}</strong><span>Batches</span><small>Assigned classes</small></div>${fourth}</div><div class="focus-strip">${focusCards.map(([label,count,caption,section,icon])=>`<button class="focus-tile" onclick="go('${section}')"><span>${icon}</span><strong>${count}</strong><small>${label}</small><em>${caption}</em></button>`).join('')}</div><div class="card quick-orbit-card"><div class="quick-orbit-scene"><div class="quick-orbit-ring"></div><div class="quick-orbit-center"><span>Quick</span><strong>Actions</strong></div><div class="quick-orbit-actions">${quickActions()}</div></div></div><div class="section-heading-row"><h3 class="section-title">Recent Notices</h3>${isAllowedSection('notices')?`<button class="secondary-btn compact-btn" onclick="go('notices')">View all</button>`:''}</div><div class="list">${freshNotices.map(noticeCard).join('')||'<div class="empty">No notices yet.</div>'}</div>`}
