...
function renderNav(){const nav=$('bottomNav');nav.innerHTML=navItems().map(([id,icon,label])=>`<button class="${state.section===id?'active':''}" onclick="go('${id}')" aria-label="Open ${label}" ${state.section===id?'aria-current="page"':''}><span class="nav-icon">${icon}</span><span>${label}</span></button>`).join('')}
